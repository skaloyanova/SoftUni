...
namespace TownApplication.IntegrationTests
{
    public class TownControllerIntegrationTests
    {
        ...
        public void AddTown_ValidInput_ShouldAddTown()
        {
            ...
            var result = _controller.GetTownByName(validTownName);  //null if town is not found
        ...
        public void AddTown_DuplicateTownName_DoesNotAddDuplicateTown()
        {
            ...
            Assert.Equal(population, town.Population);  //validate that the 1st entry is written in db and not the 2nd (with higher population)
        }
        ...
        public void UpdateTown_InvalidPopulation_ShouldThrowArgumentException(int invalidPopulation)
        {
            ...
            //Method doesn't throw exception and does updates with invalid population, which is incorrect
            //var action = () => _controller.UpdateTown(town.Id, updatedPopulation); 
            //Assert.Throws<ArgumentException>(action);

            _controller.UpdateTown(town.Id, invalidPopulation);

            var updatedTown = _controller.GetTownByName(townName);
            Assert.NotNull(updatedTown);
            Assert.Equal(initialPopulation, updatedTown.Population);  
        }
        ...
        public void ListTowns_ShouldReturnTowns()
        {
            ...
            var townsFromDatabase = _controller.ListTowns();
            ...
            Assert.NotNull(townsFromDatabase);
            Assert.Equal(townsToAdd.Count, townsFromDatabase.Count);
            foreach (var town in townsToAdd)
            {
                var retrievedTown = _controller.GetTownByName(town);
                Assert.NotNull(retrievedTown);
                Assert.Equal(town.Length * 1000, retrievedTown.Population);
            }
        }
    }
}
